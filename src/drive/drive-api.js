...
export async function findFolder(token, name, parentId = 'root') {
  ...
  const params = new URLSearchParams({ q, fields: 'files(id,name)', pageSize: '200', ...SHARED })
  const res = await fetch(`${BASE}/files?${params}`, { headers: authHeaders(token) })
...
async function resolveVCPSubfolders(token, vcp, driveName) {
  ...
  if (!inbox) throw new Error(`Found /vcp/ in "${driveName}" but no inbox subfolder`)
  return {
    vcpId: vcp.id,
    inboxId: inbox.id,
...
export async function listClients(token, clientsFolderId) {
  ...
  const params = new URLSearchParams({ q, fields: 'files(id,name)', orderBy: 'name', pageSize: '100', ...SHARED })
  const res = await fetch(`${BASE}/files?${params}`, { headers: authHeaders(token) })
